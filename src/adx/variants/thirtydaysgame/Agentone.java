...
public class Agentone extends ThirtyDaysThirtyCampaignsAgent {

  public Agentone(String host, int port) {
    super(host, port);
  }
...
//    		  adx.structures.MarketSegment x = getMktSegm(array[i]) ;
    		  if(c.getBudget()>0.1){
    		  bidEntries.add(new SimpleBidEntry(imp.get(array[i]), (c.getBudget()*pfactor*f2/(double)c.getReach()), c.getBudget()));
    		  }
...
//      return new ThirtyDaysBidBundle(day, c.getId(), Math.max(100,c.getBudget()), bidEntries);
  ...
  public static void main(String[] args) {
	  Agentone agent = new Agentone("localhost", 9898);
